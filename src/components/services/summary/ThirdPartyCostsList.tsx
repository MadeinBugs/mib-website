...
export default function ThirdPartyCostsList({ costs, locale, currency }: ThirdPartyCostsListProps) {
	...
	return (
		<div className="space-y-2">
			...
			<p className="text-[10px] text-service-text-tertiary">
				{locale === 'en'
			...
			<ul className="space-y-1">
				{costs.map((cost) => (
					<li key={cost.id} className="flex items-center justify-between text-xs">
						<span className="text-service-text-secondary">{cost.label[locale]}</span>
						<span className="text-service-text-secondary shrink-0">
